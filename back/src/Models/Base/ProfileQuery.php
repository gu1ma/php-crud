...
use \Exception;
use \PDO;
use Models\Profile as ChildProfile;
use Models\ProfileQuery as ChildProfileQuery;
use Models\Map\ProfileTableMap;
 ...
 * @method     ChildProfileQuery orderByName($order = Criteria::ASC) Order by the name column
 *
 * @method     ChildProfileQuery groupById() Group by the id column
 * @method     ChildProfileQuery groupByName() Group by the name column
 *
 ...
 *
 * @method     ChildProfileQuery leftJoinWith($relation) Adds a LEFT JOIN clause and with to the query
 * @method     ChildProfileQuery rightJoinWith($relation) Adds a RIGHT JOIN clause and with to the query
 * @method     ChildProfileQuery innerJoinWith($relation) Adds a INNER JOIN clause and with to the query
 *
 ...
 *
 * @method     ChildProfileQuery joinWithUser($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the User relation
 *
 * @method     ChildProfileQuery leftJoinWithUser() Adds a LEFT JOIN clause and with to the query using the User relation
 * @method     ChildProfileQuery rightJoinWithUser() Adds a RIGHT JOIN clause and with to the query using the User relation
 * @method     ChildProfileQuery innerJoinWithUser() Adds a INNER JOIN clause and with to the query using the User relation
 *
 ...
 * @method     ChildProfile findOneByName(string $name) Return the first ChildProfile filtered by the name column *

 * @method     ChildProfile requirePk($key, ConnectionInterface $con = null) Return the ChildProfile by primary key and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildProfile requireOne(ConnectionInterface $con = null) Return the first ChildProfile matching the query and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 * @method     ChildProfile requireOneById(int $id) Return the first ChildProfile filtered by the id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildProfile requireOneByName(string $name) Return the first ChildProfile filtered by the name column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildProfile[]|ObjectCollection findByName(string $name) Return ChildProfile objects filtered by the name column
...
abstract class ProfileQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Models\Base\ProfileQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'treinamentophp', $modelName = '\\Models\\Profile', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = ProfileTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key)))) {
    ...
    {
        $sql = 'SELECT id, name FROM profile WHERE id = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            ProfileTableMap::addInstanceToPool($obj, null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key);
        }
        $stmt->closeCursor();
    ...
    public function filterByName($name = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(ProfileTableMap::COL_NAME, $name, $comparison);
    }
    ...
    public function filterByUser($user, $comparison = null)
    {
        if ($user instanceof \Models\User) {
            return $this
                ->addUsingAlias(ProfileTableMap::COL_ID, $user->getProfileId(), $comparison);

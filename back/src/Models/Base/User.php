...
abstract class User implements ActiveRecordInterface
{
    ...
    /**
     * The value for the datelastlogin field.
     *
     * @var        string
     */
    ...
    /**
     * Get the [datelastlogin] column value.
     *
     * @return string
     */
    ...
    public function setDatecreation($v)
    {
        ...
        if ($this->datecreation !== $v) {
            $this->datecreation = $v;
            $this->modifiedColumns[UserTableMap::COL_DATECREATION] = true;
        }
    ...
    /**
     * Set the value of [datelastlogin] column.
     *
     * @param string $v new value
     * @return $this|\Models\User The current object (for fluent API support)
     */
    public function setDatelastlogin($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->datelastlogin !== $v) {
            $this->datelastlogin = $v;
            $this->modifiedColumns[UserTableMap::COL_DATELASTLOGIN] = true;
        }
    ...
    public function setProfileId($v)
    {
        ...
        if ($this->profile_id !== $v) {
            $this->profile_id = $v;
            $this->modifiedColumns[UserTableMap::COL_PROFILE_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UserTableMap::translateFieldName('Datecreation', TableMap::TYPE_PHPNAME, $indexType)];
            $this->datecreation = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UserTableMap::translateFieldName('Datelastlogin', TableMap::TYPE_PHPNAME, $indexType)];
            $this->datelastlogin = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : UserTableMap::translateFieldName('ProfileId', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_DATECREATION)) {
            $modifiedColumns[':p' . $index++]  = 'dateCreation';
        }
        if ($this->isColumnModified(UserTableMap::COL_DATELASTLOGIN)) {
            $modifiedColumns[':p' . $index++]  = 'dateLastLogin';
        }
        if ($this->isColumnModified(UserTableMap::COL_PROFILE_ID)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'email':
                        ...
                        break;
                    case 'dateCreation':
                        $stmt->bindValue($identifier, $this->datecreation, PDO::PARAM_STR);
                        break;
                    case 'dateLastLogin':
                        $stmt->bindValue($identifier, $this->datelastlogin, PDO::PARAM_STR);
                        break;
                    case 'profile_id':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[5] => $this->getDatelastlogin(),
            $keys[6] => $this->getProfileId(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[5], $arr)) {
            $this->setDatelastlogin($arr[$keys[5]]);
        }
        if (array_key_exists($keys[6], $arr)) {
            $this->setProfileId($arr[$keys[6]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_DATECREATION)) {
            $criteria->add(UserTableMap::COL_DATECREATION, $this->datecreation);
        }
        if ($this->isColumnModified(UserTableMap::COL_DATELASTLOGIN)) {
            $criteria->add(UserTableMap::COL_DATELASTLOGIN, $this->datelastlogin);
        }
        if ($this->isColumnModified(UserTableMap::COL_PROFILE_ID)) {
            $criteria->add(UserTableMap::COL_PROFILE_ID, $this->profile_id);
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setDatelastlogin($this->getDatelastlogin());
